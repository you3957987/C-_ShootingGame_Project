...
void Base::update(Uint32 deltaTime) {
    ...
    double frameRateFactor = static_cast<double>(deltaTime) / (1000.0 / 30.0); // 프레임 속도를 60으로 고정. 너무 빠르면 뚝뚝 끊겨 보임.
    double distance = moveSpeed * frameRateFactor;

    // 적의 위치 업데이트
    position.y += static_cast<int>(distance); // base특징 : 지정한 속도로 아래로 떨어지기.

    // 적이 화면을 벗어나면 다시 위로 이동
    if (position.y > screenHeight) {
        position.y = 0;
        position.x = disX(gen);
    }

    lastSpawnTime += deltaTime; // 시간 업데이트
}
